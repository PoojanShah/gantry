...
public class ContourEditor : Singleton<ContourEditor>
{
    ...
    private static Tool[] toolBehaviour = new Tool[]{//For each,MouseDown,MouseDrag,Draw,MouseUp
        new Tool{//Vertex
            OnMouseDown=(p)=>{
                if(selectionShape==Shape.lasso)AddSelectionLassoPunkt(p);
            },
            OnDrag=(p)=>{if(selectionShape==Shape.lasso)UpdateSelectionLasso(p);},
            OnFinishDrag=(p)=>{
                Debug.Log("Vertex mode's OnFinishDrag("+p+") selectionShape: "+selectionShape+",downPoint: "+downPoint+",downPoint.FlipY(): "+downPoint.FlipY());
                if(selectionShape==Shape.lasso){
                }else{
                    Vector2 startPoint=Input.GetKey(centeredSelectionKey)?p+(downPoint-p)*2:downPoint;
                    Debug.Log("Vertex mode's OnFinishDrag("+p+") startPoint: "+startPoint+",downPoint: "+downPoint+",downPoint.FlipY(): "+downPoint.FlipY());
                    /*if(dragging)*/GroupSelect(new Rect(//bandboxing
				        Mathf.Min(startPoint.x,p.x),Mathf.Min(startPoint.y,p.y),
                        Mathf.Abs(p.x-startPoint.x),Mathf.Abs(p.y-startPoint.y)));
                }
            },
            Draw=(p)=>{
                Vector2 startPoint=Input.GetKey(centeredSelectionKey)?p+(downPoint.FlipY()-p)*2:downPoint.FlipY();
                if(dragging)
                    if(selectionShape==Shape.rect)Graphics.DrawRectAround(startPoint,SRSUtilities.adjustedFlipped,Color.red);
                    else if(selectionShape==Shape.ellipse)DrawEllipseAround(startPoint,SRSUtilities.adjustedFlipped);//Kopf
            ...
            OnSingleClick=(p)=>{
                ...
                RaycastHit hit;
                if(!Physics.Raycast(Camera.main.ScreenPointToRay(SRSUtilities.adjustedMousePosition),out hit,Mathf.Infinity,1<<LayerMask.NameToLayer("Projection"))){
                    ...
                    downPoint=-Vector2.one;
                    return;
                }else if(Vertex.dragging==null){//Hit projection; Die naereste Vertex wahlen.
				    Mesh m=instance.GetComponent<MeshFilter>().mesh;
                    int nearestVertex=-1;
                    float closestDistance=Mathf.Infinity,distance;

                    for(int v=0;v<m.vertices.Length;v++)if((distance=Vector3.Distance(hit.point.Flatten(1),instance.transform.TransformPoint(m.vertices[v]).Flatten(1)))<closestDistance){
                        nearestVertex=v;
                    ...
                    Debug.Log("Nearest to hit point "+hit.point+" was "+nearestVertex+" ("+instance.transform.TransformPoint(m.vertices[nearestVertex])+") at distance :"+closestDistance+".");
                    if(!Input.GetKey(addSelectKey))DeSelect(false);
                    SelectVertex(nearestVertex);
                }
            },
            keyDowns=new Dictionary<KeyCode,Action>(){
               {KeyCode.Delete,()=>{Delete(ApplicableVertices());Debug.Log("Deleted triangles containing vertices "+ApplicableVertices().Stringify()+".");}},
        ...
        new Tool{//Blackout
            OnMouseDown=(p)=>{//Down
                ...
                if(Blackout.selected>-1&&blackouts[Blackout.selected].Contains(p.FlipY())){
                    ...
                    Debug.Log("Starting to move blackout["+Blackout.moving+"]. UndoStep's position: "+new Vector2(blackouts[Blackout.moving].rect.x,blackouts[Blackout.moving].rect.y));
                }else if(Blackout.shape==Shape.lasso)for(int i=0;i<2;i++)AddLassoPoint(p,Color.black);
            },
            Draw=(p)=>{//Draw
                if(Blackout.moving<0)switch(Blackout.shape){
                    case Shape.rect:
                        if(dragging)Graphics.DrawBox(SRSUtilities.RectAround(Input.GetKey(centeredSelectionKey)?SRSUtilities.adjustedFlipped+(downPoint.FlipY()-SRSUtilities.adjustedFlipped)*2:downPoint.FlipY(),SRSUtilities.adjustedFlipped),new Color(0,0,0,1));
                        break;
                    case Shape.ellipse:
                        if(dragging)Graphics.DrawFilledEllipse(SRSUtilities.RectAround(Input.GetKey(centeredSelectionKey)?SRSUtilities.adjustedFlipped+(downPoint.FlipY()-SRSUtilities.adjustedFlipped)*2:downPoint.FlipY(),SRSUtilities.adjustedFlipped),new Color(0,0,0,1));
                        break;
                    case Shape.lasso:
                        if(Input.GetKey(createLassoBlackoutKey)&&lassoBlackout!=null)FinishBuildingLassoBlackout();
                        break;
                    default:
            ...
            OnDrag=(p)=>{
                if(Blackout.moving>-1){
                    Debug.Log("Lasso mode's OnDrag; moving: "+Blackout.moving+", lassoObj: "+(blackouts[Blackout.moving].lassoObject!=null));
            ...
            OnFinishDrag=(p)=>{//Up
                ...
                else if(Blackout.shape!=Shape.lasso){//Wasn't moving a blackout,created a new one.
	                blackouts.Add(new Blackout{rect=SRSUtilities.RectAround(Input.GetKey(centeredSelectionKey)?SRSUtilities.adjustedFlipped+(downPoint.FlipY()-SRSUtilities.adjustedFlipped)*2:downPoint.FlipY(),SRSUtilities.adjustedFlipped),elliptical=Blackout.shape==Shape.ellipse});//Adding a blackout
	                AddUndoStep(/*/*new UndoStep{createBlackout=true,blackout=blackouts.Last(),blackoutInd=blackouts.Count-1}*/);
            ...
            OnSingleClick=(p)=>{
                //RaycastHit hit;
                int b;
                for(b=blackouts.Count-1;b>-1;b--)if(blackouts[b].Contains(SRSUtilities.adjustedFlipped)){
                        Blackout.Select(b);
                        break;
                }
                if(b==-1){
        ...
        new Tool{//Whiteout
            OnMouseDown=(p)=>{//Down
                ...
                if(Blackout.selected>-1&&blackouts[Blackout.selected].Contains(p.FlipY())){
                    blackouts[Blackout.moving=Blackout.selected].SetOffsetFrom(p.FlipY());
                    Debug.Log("Starting to move blackout["+Blackout.moving+"]. UndoStep's position: "+new Vector2(blackouts[Blackout.moving].rect.x,blackouts[Blackout.moving].rect.y));
                }else if(Blackout.shape==Shape.lasso)for(int i=0;i<2;i++)AddLassoPoint(p,Color.white);
            },
            Draw=(p)=>{//Draw
                if(Blackout.moving<0)switch(Blackout.shape){
                    case Shape.rect:
                        if(dragging)Graphics.DrawBox(SRSUtilities.RectAround(Input.GetKey(centeredSelectionKey)?SRSUtilities.adjustedFlipped+(downPoint.FlipY()-SRSUtilities.adjustedFlipped)*2:downPoint.FlipY(),SRSUtilities.adjustedFlipped),Color.white);
                        break;
                    case Shape.ellipse:
                        if(dragging)Graphics.DrawFilledEllipse(SRSUtilities.RectAround(Input.GetKey(centeredSelectionKey)?SRSUtilities.adjustedFlipped+(downPoint.FlipY()-SRSUtilities.adjustedFlipped)*2:downPoint.FlipY(),SRSUtilities.adjustedFlipped),Color.white);
                        break;
                    case Shape.lasso:
                        if(Input.GetKey(createLassoBlackoutKey)&&lassoBlackout!=null)FinishBuildingLassoBlackout();
                        break;
            ...
            OnFinishDrag=(p)=>{//Up
                if(Blackout.moving>-1)Blackout.moving=-1;
	                ...
	                blackouts.Add(new Blackout{rect=SRSUtilities.RectAround(Input.GetKey(centeredSelectionKey)?SRSUtilities.adjustedFlipped+(downPoint.FlipY()-SRSUtilities.adjustedFlipped)*2:downPoint.FlipY(),SRSUtilities.adjustedFlipped),elliptical=Blackout.shape==Shape.ellipse,farbe=Color.white});//Adding a blackout
	                AddUndoStep(/*/*new UndoStep{createBlackout=true,blackout=blackouts.Last(),blackoutInd=blackouts.Count-1}*/);
                }//else{//Added a lasso punkt
                //}
            },
            OnSingleClick=(p)=>{
                int b;
                for(b=blackouts.Count-1;b>-1;b--)if(/*!blackouts[i].deleted&&*/blackouts[b].Contains(SRSUtilities.adjustedFlipped)){
                        Blackout.Select(b);
                        break;
                }
                if(b==-1){
        ...
        new Tool{//Scale
            OnMouseDown=(p)=>{if(ScaleZones().Any((r)=>r.Contains(p,true)))BeginScale(p,scaleMode);},
            OnDrag=(p)=>{//Update
			    Vector3[] verts=instance.GetComponent<MeshFilter>().mesh.vertices;
                Vector2 clampMin=new Vector2(1-(convergingEdgeScreen.x-oppositeScreenEdge.x)/(convergingEdgeScreen.x-oppositeEdgeScreen.x),1-(convergingEdgeScreen.y-oppositeScreenEdge.y)/(convergingEdgeScreen.y-oppositeEdgeScreen.y)),
                clampMax=Vector2.one;
                for(int i=verts.Length-1;i>-1;i--)if(!deleted.Contains(i)){
                    //float perpendicularProximity=Mathf.Abs(scalePivot.y-(float)(i/columns)/(float)columns);
                    float perpendicularProximity=(float)(scaleMode==ScaleMode.horizontal?(i/columns):(i%columns))/(float)columns;
                    if(scaleMode==ScaleMode.vertical)perpendicularProximity=1-perpendicularProximity;
                    float pivot=scaleMode==ScaleMode.normal?-1:scalePivot[2-(int)scaleMode];//We have not explored all possible implications of the -1. It works for now.
                    ...
                    if(i==22||i==40)Debug.Log("pivot: "+pivot+", perp: "+perpendicularProximity+", scaleSnapshot["+i+"].z: "+scaleSnapshot[i].z+", convergingEdgeWorld.y: "+convergingEdgeWorld.y+", tween: "+Mathf.Clamp((SRSUtilities.adjustedMousePosition.y-downPoint.y)/(convergingEdgeScreen.y-downPoint.y),clampMin.y,clampMax.y));
                    verts[i]=new Vector3(scalePivot.x<0?verts[i].x:SRSUtilities.Lerp(scaleSnapshot[i].x,convergingEdgeWorld.x,Mathf.Clamp((SRSUtilities.adjustedMousePosition.x-downPoint.x)/(convergingEdgeScreen.x-downPoint.x),clampMin.x,clampMax.x)*(scaleMode==ScaleMode.horizontal?perspectiveFactor:1)),//scalePivot range is [0,1]
				                         verts[i].y,
                                         scalePivot.y<0?verts[i].z:SRSUtilities.Lerp(scaleSnapshot[i].z,convergingEdgeWorld.y,Mathf.Clamp((SRSUtilities.adjustedMousePosition.y-downPoint.y)/(convergingEdgeScreen.y-downPoint.y),clampMin.y,clampMax.y)*(scaleMode==ScaleMode.vertical?perspectiveFactor:1)));
            ...
            Draw=(p)=>{
                ...
                if(scaleMode==ScaleMode.normal){
                    ...
                    for(int i=0;i<scaleZones.Length;i++)if(!movingScaleZones.Contains(i))Graphics.DrawRect(scaleZones[i],Color.magenta);
                    for(int i=0;i<scaleZones.Length;i++)if(movingScaleZones.Contains(i))Graphics.DrawRect(scaleZones[i],Color.cyan);
                }else DrawPerspectiveZones(scaleMode==ScaleMode.horizontal?Color.yellow:new Color(1,0.5f,0,1));
            },
        }
    };
    ...
    public enum ToolMode { vertex = 0, blackout, scale };
    ...
    {
        if (lassoPoints.Count < 1) lassoPoints.Add(Camera.main.ScreenToWorldPoint(p) + Camera.main.transform.forward);//Repeat the first one to start wit zwei.
        lassoPoints.Add(Camera.main.ScreenToWorldPoint(p) + Camera.main.transform.forward);
        lines.Add((GameObject.Instantiate(instance.lassoLine.gameObject, Vector3.zero, Quaternion.identity) as GameObject).GetComponent<LineRenderer>());
        ...
        Debug.Log("AddSelectionLassoPunkt(" + p + ") lines.Count: " + lines.Count + ",lassoPoints.Count: " + lassoPoints.Count);
    ...
    public class Blackout
    {
        ...
        public static void Select(int b)
        {
            ...
            if (selected > -1 && blackouts[selected].lassoObject != null) blackouts[selected].lassoObject.GetComponent<MeshRenderer>().material.color = selectedBlackoutColor;
        ...
        public bool Contains(Vector2 p)
        {
            ...
            {//lassoed Blackout
                ...
                for (int t = 0; t < triangles.Length - 3; t++) if (SRSUtilities.PointInTriangle(p.FlipY(),
                              lassoObject.transform.InverseTransformPoint(Camera.main.WorldToScreenPoint(verts[triangles[t]])),
                              lassoObject.transform.InverseTransformPoint(Camera.main.WorldToScreenPoint(verts[triangles[t + 1]])),
                              lassoObject.transform.InverseTransformPoint(Camera.main.WorldToScreenPoint(verts[triangles[t + 2]])))) return true;
                return false;
            }
        ...
        {
            get { return lassoObject != null ? Camera.main.WorldToScreenPoint(lassoObject.transform.position).ToVector2XZ() : rect.position; }
            set
            {
                Debug.Log("set screen position - was: " + rect.position + ", now: " + value);
                if (lassoObject != null)
                {
                    Vector3 p3d = lassoObject.transform.InverseTransformPoint(Camera.main.ScreenToWorldPoint(value));
        ...
        public static void MoveSelectedBy(Vector2 by, bool addUndo = true)
        {
            ...
            if (blackouts[selected].lassoObject != null)
            {
                ...
                for (int i = 0; i < verts.Length; i++) verts[i] += Camera.main.ScreenToWorldPoint(by) - Camera.main.ScreenToWorldPoint(Vector2.zero);
                blackouts[selected].lassoObject.GetComponent<MeshFilter>().mesh.vertices = verts;
            }
            else blackouts[selected].screenPosition += by;
            if (addUndo) AddUndoStep(/*new UndoStep{moveBlackout=true,blackout=blackouts[selected],blackoutInd=selected,delta=blackouts[selected].screenPosition,
        ...
        {
            if (selected > -1 && blackouts.Count > selected && blackouts[selected].lassoObject != null) blackouts[selected].lassoObject.GetComponent<MeshRenderer>().material.color = blackouts[selected].farbe.WithAlpha(1);
            selected = -1;
        }

        public static void Delete(int b, bool addUndo = true)
        {
            Debug.Log("ContourEditor.Blackout.Delete(" + b + "," + addUndo + ") lassoObject not null: " + (blackouts[b].lassoObject != null));
    ...
    private void Awake()
    {
        Debug.Log("Projection.Awake(); columns: " + columns + ",Settings.dataPath: " + Settings.dataPath + ",Application.persistentDataPath: " + Application.persistentDataPath);
        ...
        foreach (GameObject o in new GameObject[] { lassoPoint, lassoLine.gameObject }) Graphics.SetAlpha(o.GetComponent<Renderer>().material, editingLassoAlpha);
        ResetLassoVisuals();
        ...
        for (int s = 0; s < shapes.Length; s++)
        {
            ...
            toolMenu[0][s] = new ToolbarMenu.Item()
            {
                ...
                shortcut = new KeyCode[][] { new KeyCode[] { KeyCode.Keypad7 }, new KeyCode[] { KeyCode.Keypad8 }, new KeyCode[] { KeyCode.Keypad9 } }[s],
                info = new string[]{
            ...
            toolMenu[1][s] = new ToolbarMenu.Item()
            {
                ...
                shortcut = new KeyCode[][] { new KeyCode[] { KeyCode.Keypad4 }, new KeyCode[] { KeyCode.Keypad5 }, new KeyCode[] { KeyCode.Keypad6 } }[s],
                info = new string[]{
            ...
            toolMenu[2][s] = new ToolbarMenu.Item()
            {
                ...
                shortcut = new KeyCode[][] { new KeyCode[] { KeyCode.KeypadDivide }, new KeyCode[] { KeyCode.KeypadMultiply }, new KeyCode[] { KeyCode.KeypadMinus } }[s],
                info = new string[]{
            ...
            toolMenu[3][s] = new ToolbarMenu.Item()
            {
                ...
                shortcut = new KeyCode[][] { new KeyCode[] { KeyCode.Keypad1 }, new KeyCode[] { KeyCode.Keypad2 }, new KeyCode[] { KeyCode.Keypad3 } }[s],
                info = new string[]{
        ...
        for (int i = 0; i < toolMenu.Length; i++) for (int j = 0; j < toolMenu[i].Length; j++) toolMenu[i][j].buttonContent.tooltip = toolMenu[i][j].info.Split(new string[] { "\n" }, StringSplitOptions.None)[0];
        ...
        for (int i = 0; i < backgrounds.Length; i++)
        {
            ...
            {
                buttonContent = new GUIContent(backgrounds[i], SRSUtilities.CapFirsts(Regex.Replace(backgrounds[i].name, "_", " "))),
        ...
        ToolbarMenu.Item[][] actionMenu = new ToolbarMenu.Item[][]{
            new ToolbarMenu.Item[]{//File Menu
                new ToolbarMenu.Item(){buttonContent=new GUIContent("File","File Operations")},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Save File"),"Save Configuration"),OnSelect=()=>{mode=Mode.save;Menu.ResetWindowPosition();saveAsDefault=-1;SRSUtilities.guiMatrixNormalized=false;},shortcut=new KeyCode[]{KeyCode.S}},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Open File"),"Load Configuration"),OnSelect=()=>{mode=Mode.load;Menu.ResetWindowPosition();SRSUtilities.guiMatrixNormalized=false;},shortcut=new KeyCode[]{KeyCode.L}},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Reset"),"Restart the Editor"),OnSelect=()=>{Reset(-1,true,true);Restart();}},
				//{new GUIContent("Information"),()=>infoOn=!infoOn},
				new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Quit"),"Quit to Menu"),OnSelect=SaveAndQuitToMenu}
            },
            new ToolbarMenu.Item[]{//Edit Menu
                new ToolbarMenu.Item(){buttonContent=new GUIContent("Edit","Edit Operations")},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Undo"),"Undo Last Action"),OnSelect=()=>Undo(-1),shortcut=new KeyCode[]{KeyCode.Z},disabled=true},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Redo"),"Redo Undone Action"),OnSelect=()=>Undo(1),shortcut=new KeyCode[]{KeyCode.D},disabled=true},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Select All"),"Select All"),OnSelect=SelectAll,shortcut=new KeyCode[]{KeyCode.A}}
            },
            new ToolbarMenu.Item[]{//View Menu
                new ToolbarMenu.Item(){buttonContent=new GUIContent("View","View Options")},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("X Mirror"),"Mirror Horizontally"),OnSelect=()=>ToggleMirror(0),shortcut=new KeyCode[]{KeyCode.X}},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Y Mirror"),"Mirror Vertically"),OnSelect=()=>ToggleMirror(1),shortcut=new KeyCode[]{KeyCode.Y}},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Circle Mode"),"Edit in a Circle"),OnSelect=()=>circle=!circle,shortcut=new KeyCode[]{KeyCode.C}},
                new ToolbarMenu.Item(){buttonContent=new GUIContent(icons.ByName("Information"),"Show/Hide Information"),OnSelect=()=>instance.toolbar.ToggleInfo(),shortcut=new KeyCode[]{KeyCode.I}}
            }
        };
        ...
        toolbar.SetMenu(new ToolbarMenu[]{
            new ToolbarMenu(toolMenu,true,(s)=>{
                toolMode=(ToolMode)s;
                ResetTools();
                Debug.Log("Changing toolmenu #0 to: "+s+". selectedCategory previously: "+toolbar.menus[0].selectedCategory);
                toolbar.menus[2].ItemByTooltip("Mirror Horizontally").disabled=
                    toolbar.menus[2].ItemByTooltip("Mirror Vertically").disabled=s!=0&&s!=3;//toolbar.menus[0].selectedCategory!=0&&toolbar.menus[0].selectedCategory!=3;
                toolbar.menus[2].ItemByTooltip("Edit in a Circle").disabled=
                    toolbar.menus[2].ItemByTooltip("Select All").disabled=s!=0;//toolbar.menus[0].selectedCategory==0;//Mirror and circle modes only affect vertex displacement; they therefore only make sense in that category.
    ...
    public static void AddUndoStep(bool resetCurrentStep = true)
    {
        ...
        for (int i = 0; i < blackouts.Count; i++) if (blackouts[i].lassoObject != null)
            {
                us.blackoutLassoMeshes[i] = blackouts[i].lassoObject.GetComponent<MeshFilter>().mesh.vertices;
                us.blackoutTriangleSnapshots[i] = blackouts[i].lassoObject.GetComponent<MeshFilter>().mesh.triangles;
            }
        undos.Add(us);
        if (resetCurrentStep) undo = undos.Count - 1;
        UpdateUndoMenu();
        Debug.Log("Projection.AddUndoStep(" + resetCurrentStep + ") ANSCHLUSS, undo: " + undo + "/" + undos.Count + ", step:\n" + us.Stringify());
    }
    ...
    public static void Undo(int dir)
    {
        ...
        undo += dir;
        UpdateUndoMenu();
        Debug.LogWarning("ContourEditor.Undo(" + dir + "), undo jetzt: " + undo + "/" + undos.Count + ", vor: " + (undo - dir) + " Treppe:\n" + undos[undo].Stringify() + ", deleted.Count: " + deleted.Count);
        //Reset();
        WipeIntermittents();
        WipeBlackouts();
        ...
        Debug.LogWarning("VOR deleted.Count: " + deleted.Count + ", undos[undo].deletedVerts.Count: " + (undos[undo].deletedVerts.Length));
        ...
        for (int i = 0; i < vertexDots.Count; i++) if (deleted.Contains(i))
            {
        ...
        for (int i = 0; i < undos[undo].blackouts.Length; i++) if (undos[undo].blackoutLassoMeshes[i] != null)
            {
                ...
                mesh.RecalculateNormals();
                mesh.RecalculateBounds();
                blackouts[i].lassoObject.GetComponent<MeshRenderer>().material.color = blackouts[i].lassoObject.GetComponent<MeshRenderer>().material.color.WithAlpha(1);
            }
    }
    ...
    {//Start Anew.
        ...
        //mesh.RecalculateNormals();
        //mesh.RecalculateBounds();
        if (instance.toolbar != null && instance.toolbar.menus != null && instance.toolbar.menus.Length > 0) instance.toolbar.menus[0].SelectItem(0, 0);
    ...
    {//Delta -1 to halve,0 to maintain,1 to double.
        ...
        columns = delta > 1 ? delta : Mathf.Max(minDensity, Mathf.Min(maxDensity, (int) ((columns - 1) * Mathf.Pow(2, delta) + 1)));
        Debug.Log("Projection.ReconstructScreen(" + delta + "), columns vor: " + oldColumns + ",delta: " + delta + ", jetzt: " + columns);
        #region Vertices		
        ...
        for (int z = 0; z < columns; z++)
            for (int x = 0; x < columns; x++)
                if (!omitDeleted || !deleted.Contains(z * columns + x))
                {
                    if (delta != 0)
                    {
                        int ox1 = x * (oldColumns - 1) / (columns - 1), ox2 = Mathf.Min(oldColumns - 1, ox1 + 1),
                            oz1 = z * (oldColumns - 1) / (columns - 1), oz2 = Mathf.Min(oldColumns - 1, oz1 + 1);
                        float total = (float) (columns - 1) / (float) (oldColumns - 1);//(vertices.Length-1)/(oldVerts.Length-1);
                        vertices[x + z * columns] = Vector3.Lerp(
                            Vector3.Lerp(oldVerts[ox1 + oz1 * oldColumns], oldVerts[ox2 + oz1 * oldColumns], x % total / total),
        ...
        for (int v = 0; v < columns; v++) for (int u = 0; u < columns; u++) uvs[u + v * columns] = new Vector2((float) u / (columns - 1), (float) v / (columns - 1));
        #endregion
        #region Triangles
        int nbFaces = (columns - 1) * (columns - 1);
        int[] triangles = new int[nbFaces * 6];
        //		int t=0;
        for (int face = 0; face < nbFaces; face++) AddFace(triangles, face);//Retrieve lower left corner from face ind
        #endregion
    ...
    {
        return new bool[] { mirror[0] && SideX(vertexDots.IndexOf(Vertex.dragging)) != SideX(vi), mirror[1] && SideY(vertexDots.IndexOf(Vertex.dragging)) != SideY(vi) };
    }
    public void MoveSelectedVerticesBy(Vector3 differential, bool flipY = false)
    {
        ...
        foreach (int v in applicableVertices)
        {
            ...
            differential.x = Mathf.Max(Mathf.Min(differential.x, rawSize.x - vertices[v].x * (mirroredOpposite[0] ? -1 : 1)), -rawSize.x - vertices[v].x * (mirroredOpposite[0] ? -1 : 1));
            differential.z = Mathf.Max(Mathf.Min(differential.z, transform.InverseTransformPoint(Projection.originalExtents).z - vertices[v].z * (mirroredOpposite[1] ? -1 : 1)), -transform.InverseTransformPoint(Projection.originalExtents).z - vertices[v].z * (mirroredOpposite[1] ? -1 : 1));
        }
        applicableVertices.ToList().ForEach((vi) =>
        {
            ...
            vertices[vi] = new Vector3(vertices[vi].x + diff.x, Vertex.y, vertices[vi].z + diff.z * (flipY ? -1 : 1));
        ...
        selectedVertices.ForEach((i) =>
        {
            ...
            vertexDots[i].transform.position += new Vector3(differential.x * (mirroredOpposite[0] ? -1 : 1), differential.y, differential.z * (mirroredOpposite[1] ? -1 : 1));
        });
    ...
    private static int[] ApplicableVertices()
    {
        ...
        foreach (int vert in selectedVertices)
        {
            ...
            if (circle)
            {
                int[] arc = SRSUtilities.ArcPattern(SRSUtilities.RadiusOf(Mathf.Abs(columns / 2 - vert % columns), Mathf.Abs(columns / 2 - vert / columns)));
                int v;
            ...
            if (mirror[0] || circle) foreach (int i in neue.ToArray()) neue.AddUnique(XMirror(i));
            if (mirror[1] || circle) foreach (int i in neue.ToArray()) neue.AddUnique(YMirror(i));
    ...
    {
        return (instance.GetComponent<MeshFilter>().mesh.vertices.Length / columns - i / columns - 1) * columns + i % columns;
    }
    ...
    {
        Debug.Log("ContourEditor.Delete(" + deletees.Stringify() + "), children: " + instance.transform.childCount);//Deleted triangles containing vertex "+selectedVertex+".");
        screen = screen ?? instance.gameObject;
        int[] oldTriangles = screen.GetComponent<MeshFilter>().mesh.triangles, newTriangles = new int[oldTriangles.Length - 3 * deletees.Length];
        for (int o = 0, n = 0; o < oldTriangles.Length; o += 3)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                if (omit = (deletees.Contains(oldTriangles[o + j]) ||
                         (oldTriangles[o] < columns * (columns - 1) && j == 2 && o / 3 % 2 == 0 &&//Adjacent triangles
            ...
            /*else*/
            if (!omit) for (int j = 0; j < 3; j++) {/*Debug.Log("o: "+o+",n: "+n+",j: "+j+",newTriangles.Length: "+newTriangles.Length+",oldTriangles.Length: "+oldTriangles.Length);*/newTriangles[n++] = oldTriangles[o + j]; }
        }
        screen.GetComponent<MeshFilter>().mesh.triangles = newTriangles;
        deleted.AddRangeUnique(deletees.ToList());
        for (int i = 0; i < vertexDots.Count; i++) if (!newTriangles.Contains(i)) vertexDots[i].gameObject.SetActive(false);
        DeSelect(false);
        if (addUndo) AddUndoStep(/*new UndoStep{deleteTrianglePoints=oldTriangles,selectVerts=selectedVertices.ToArray()}*/);
    }
    public static void DeSelect(bool undoStep = true)
    {
        ...
        if (undoStep/*&&selectedVertices.Count>0*/) AddUndoStep(/*new UndoStep{ selectVerts=selectedVertices.Count>0 ? selectedVertices.ToArray() : new int[]{ -1 } }*/);
    }
    public static Vector2 downPoint = -Vector2.one;//Lesson learned: use one variable for one purpose. Optimize later _IF NEED BE_.
    private static int[] movingScaleZones = new int[2] { -1, -1 };
    private void MouseDown()
    {
        Debug.Log("Projection.MouseDown()");
        if (Draggable2D.draggingAnything) return;//See that the script that inherits from Draggable2D is called prior to this in the Update queue.
        downPoint = (Vector2) SRSUtilities.adjustedMousePosition;
        if (currentTool.OnMouseDown != null) currentTool.OnMouseDown(downPoint);
        Debug.Log("Projection.MouseDown() downPoint: " + downPoint + ",SRSUtilities.adjustedMousePosition: " + SRSUtilities.adjustedMousePosition + ",flipped: " + SRSUtilities.adjustedFlipped + ",adjusted: " + SRSUtilities.adjustedMousePosition + ",adjustedFlipped: " + SRSUtilities.adjustedFlipped + ",screen dimensions: " + Screen.width + "," + Screen.height);
    }
    ...
    private static void AddLassoPoint(Vector2 p, Color c)
    {
        ...
        if (lassoBlackout == null) blackouts.Add(lassoBlackout = new Blackout() { farbe = c.WithAlpha(editingLassoAlpha) });
        lassoPoints.Add(Camera.main.ScreenToWorldPoint(p) + Camera.main.transform.forward);
        UpdateLasso();
        Debug.Log("Projection.AddLassoPoint(" + p + ") anschluß. lassoBlackout: " + lassoBlackout + ",lassoPoints.Count: " + lassoPoints.Count);
    }
    private static void MoveLastLassoPoint(Vector2 p)
    {
        ...
        {
            Debug.LogError("MoveLastLassoPoint called with no lassoBlackout. lassoBlackout: " + lassoBlackout + ",lassoPoints.Count: " + lassoPoints.Count);
            return;
        }
        lassoPoints[lassoPoints.Count - 1] = Camera.main.ScreenToWorldPoint(p) + Camera.main.transform.forward;
        UpdateLasso();
    ...
    private static void UpdateLasso()
    {
        ...
        else if (lassoPoints.Count == 2) for (int i = 0; i < 2; i++) instance.lassoLine.SetPosition(i, lassoPoints[i]);
    ...
    {
        GameObject lassoObject = new GameObject("Lasso Object", typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider));
    ...
    private static void BuildLassoMesh(Blackout blackout, Vector3[] lassoPoints)
    {
        ...
        if (blackout.lassoObject == null) blackout.lassoObject = CreateLassoObject(blackout.farbe/*.WithAlpha(editingLassoAlpha)*/);
        ...
        //lassoPoints.Clear();
        Debug.Log("lassoPoints.Count: " + lassoPoints.Length + ",mesh.vertices.Length: " + mesh.vertices.Length + ",0: " + mesh.vertices[0] + ",1: " + mesh.vertices[1] + ",2: " + mesh.vertices[2] + ",3: " + mesh.vertices[3]);
    ...
    private static void GroupDeSelect(int[] verts)
    {
        ...
        for (int i = 0; i < verts.Length; i++) if (verts[i] > -1) DeselectVertex(verts[i]);
    }
    private static void GroupSelect(int[] newVerts, bool deSelect = true)
    {
        ...
        for (int i = 0; i < newVerts.Length; i++) if (newVerts[i] > -1) SelectVertex(newVerts[i]);
    }
    private static void GroupSelect(Rect r)
    {
        ...
        switch (selectionShape)
        {
            case Shape.rect:
                ...
                r.width += 8;
                r.height += 8;
                for (int i = 0; i < vertexDots.Count; i++) if (r.Contains(Camera.main.WorldToScreenPoint(vertexDots[i].transform.position)) && !deleted.Contains(i)) SelectVertex(i);
                break;
            case Shape.ellipse:
                for (int i = 0; i < vertexDots.Count; i++) if (r.EllipseContains(Camera.main.WorldToScreenPoint(vertexDots[i].transform.position)) && !deleted.Contains(i)) SelectVertex(i);
                break;
            default:
    ...
    {
        get { return downPoint != -Vector2.one && Vector2.Distance(downPoint, SRSUtilities.adjustedMousePosition) > groupSelectThreshold; }
        set { downPoint = value ? SRSUtilities.adjustedMousePosition : -Vector2.one; }
    }
    private static Tool currentTool { get { return toolBehaviour[(int) toolMode]; } }
    private void MouseUp()
    {
        Debug.Log("Projection.MouseUp(); downPoint: " + downPoint + ",beyond threshold: " + (Vector2.Distance(downPoint, SRSUtilities.adjustedMousePosition) > groupSelectThreshold) + ",distance: " +
                  Vector2.Distance(downPoint, SRSUtilities.adjustedMousePosition) + ",threshold: " + groupSelectThreshold + ",adjustedMousePosition: " + SRSUtilities.adjustedMousePosition/*+",didMouseDownSpecial: "+didMouseDownSpecial*/);
        if (dragging)
        {
        ...
        {//single-clicking
            Debug.Log("Single-clicking. downpoint: " + downPoint + ",adjustedMousePosition: " + SRSUtilities.adjustedMousePosition + ",distance: " + Vector2.Distance(downPoint, SRSUtilities.adjustedMousePosition) + ",groupSelectThreshold: " + groupSelectThreshold + ",Projection layer: " + LayerMask.NameToLayer("Projection"));
    ...
    public enum Shape {/*cross,*/rect, ellipse, lasso };
    //private static Shape selectionMode=Shape.rect;
    public static Shape selectionShape = Shape.rect;
    private enum ScaleMode { normal = 0, horizontal, vertical };
    ...
    private static Dictionary<KeyCode, Action> universalKeyDowns = new Dictionary<KeyCode, Action>(){
       {KeyCode.B,()=>{instance.GetComponent<Renderer>().material.mainTexture=instance.backgrounds[background=(background+1)%instance.backgrounds.Length];}},
    ...
    {
        DeSelect();
        for (int i = instance.GetComponent<MeshFilter>().mesh.vertices.Length - 1; i > -1; i--) if (!deleted.Contains(i)) SelectVertex(i);
    }
    ...
    private static void BeginScale(Vector2 p, ScaleMode mode = ScaleMode.normal)
    {
        ...
        scalePivot = Vector2.one - new Vector2(scaleZones[2].Contains(SRSUtilities.adjustedFlipped, true) || scaleZones[3].Contains(SRSUtilities.adjustedFlipped, true) ? Mathf.Round((p.x - screenRectSnapshot.x) / screenRectSnapshot.width) : 2,
                                           scaleZones[0].Contains(SRSUtilities.adjustedFlipped, true) ? 1 : (scaleZones[1].Contains(SRSUtilities.adjustedFlipped, true) ? 0 : 2));
        ...
        movingScaleZones = new int[2] { -1, -1 };
        for (int i = 0, n = 0; i < scaleZones.Length; i++) if (scaleZones[i].Contains(SRSUtilities.adjustedFlipped, true)) movingScaleZones[n++] = i;
        scaleSnapshot = instance.GetComponent<MeshFilter>().mesh.vertices;
        oppositeEdgeScreen = new Vector2(screenRectSnapshot.x + screenRectSnapshot.width * (1 - scalePivot.x), screenRectSnapshot.y + screenRectSnapshot.height * (1 - scalePivot.y));
        Vector3[] verts = instance.GetComponent<MeshFilter>().mesh.vertices;
        ...
        switch (mode)
        {
            case ScaleMode.vertical://Vertically-converging Perspective
                ...
                convergingEdgeWorld = new Vector2(rendererBounds.center.x, rendererBounds.center.z) + Vector2.up * (!mirror[1] ? (rendererBounds.extents.z * (Input.mousePosition.y > Camera.main.WorldToScreenPoint(rendererBounds.center).y ? -1 : 1)) : 0);
            ...
            case ScaleMode.horizontal://Horizontal-converging Perspective
                convergingEdgeWorld = new Vector2(rendererBounds.center.x, rendererBounds.center.z) + Vector2.right * (!mirror[0] ? (rendererBounds.extents.x * (Input.mousePosition.x > Camera.main.WorldToScreenPoint(rendererBounds.center).x ? -1 : 1) * 0.75f) : 0);
                //convergingScreenEdge=Camera.main.WorldToScreenPoint(rendererBounds.center);
                convergingEdgeScreen = screenRectSnapshot.center/*+Vector2.right*(mirror[0]?(screenRectSnapshot.size.x*0.5f*(Input.mousePosition.x>Camera.main.WorldToScreenPoint(rendererBounds.center).x?-1:1)):0)*/;
                break;
            case ScaleMode.normal:
                convergingEdgeWorld = new Vector2(scalePivot.x == 1 ? verts.Max((v) => v.x) : verts.Min((v) => v.x), scalePivot.y == 0 ? verts.Max((v) => v.z) : verts.Min((v) => v.z));
                //convergingScreenEdge=new Vector2(Screen.width*scalePivot.x,Screen.height*(1-scalePivot.y));
                convergingEdgeScreen = new Vector2(screenRectSnapshot.x + screenRectSnapshot.width * scalePivot.x, screenRectSnapshot.y + screenRectSnapshot.height * scalePivot.y);
                break;
            default:
        ...
        Debug.Log("Projection.BeginScale(" + p + "," + mode + "); full: " + Mathf.Round((p.FlipY().y - screenRectSnapshot.FlipY().y) / screenRectSnapshot.height) + ",condition: " + (scaleZones[0].Contains(SRSUtilities.adjustedFlipped, true) || scaleZones[1].Contains(SRSUtilities.adjustedFlipped, true)) + ",pfy: " + p.y + ",sfy: " + screenRectSnapshot.FlipY().y + ",sfh: " + screenRectSnapshot.height + " Rects: (" + scaleZones[0].Contains(SRSUtilities.adjustedFlipped, true) + "," + scaleZones[1].Contains(SRSUtilities.adjustedFlipped, true) + "," + scaleZones[2].Contains(SRSUtilities.adjustedFlipped, true) + "," + scaleZones[3].Contains(SRSUtilities.adjustedFlipped, true) + "),scalePivot: " + scalePivot + ",convergingEdgeScreen: " + convergingEdgeScreen + ",screenRect: " + screenRectSnapshot + ",screen dimensions: (" + Screen.width + "," + Screen.height + ")");
    }
    ...
    {
        AddUndoStep(/*new UndoStep{ createBlackout=true,blackout=lassoBlackout,blackoutInd=blackouts.IndexOf(lassoBlackout) }*/);
    ...
    private void Update()
    {
        if (mode != Mode.normal || toolbar.Contains(SRSUtilities.adjustedFlipped) || Toolbar.clickedThisFrame) return;
        ...
        if (toolBehaviour[(int) toolMode].keyDowns != null) foreach (KeyValuePair<KeyCode, Action> kvp in toolBehaviour[(int) toolMode].keyDowns) if (Input.GetKeyDown(kvp.Key)) kvp.Value();
        foreach (KeyValuePair<KeyCode, Action> kvp in universalKeyDowns) if (Input.GetKeyDown(kvp.Key)) kvp.Value();
        ...
        if (toolMode == ToolMode.blackout && Input.GetKeyDown(buildLassoKey) && lassoBlackout != null) FinishBuildingLassoBlackout(lassoPoints.Count > 2);
    ...
    private static Vector2 convergingEdgeScreen, oppositeEdgeScreen;//The gantry edge/corner to which we're converging.
                                                                    //	private static Vector3[] scaleSnapshot;
    private static bool scaling { get { return scalePivot != -Vector2.one * 2; } set { if (!value) scalePivot = -Vector2.one * 2; } }
    ...
    private static Rect ScreenRect()
    {
        ...
        return SRSUtilities.RectAround(
            Camera.main.WorldToScreenPoint(new Vector3(vertexDots.Min((vd) => vd.transform.position.x), vertexDots.Min((vd) => vd.transform.position.y), vertexDots.Max((vd) => vd.transform.position.z))),
            Camera.main.WorldToScreenPoint(new Vector3(vertexDots.Max((vd) => vd.transform.position.x), vertexDots.Max((vd) => vd.transform.position.y), vertexDots.Min((vd) => vd.transform.position.z)))
            );
    }
    private static Rect[] ScaleZones()
    {
        ...
        return new Rect[]{
            SRSUtilities.RectAround(new Vector2(topLeft.x,Mathf.Lerp(topLeft.y,botRight.y,0.75f)).FlipY(),botRight.FlipY()),//Kopf
			SRSUtilities.RectAround(topLeft.FlipY(),new Vector2(botRight.x,Mathf.Lerp(topLeft.y,botRight.y,0.25f)).FlipY()),//Tief
			SRSUtilities.RectAround(topLeft.FlipY(),new Vector2(Mathf.Lerp(topLeft.x,botRight.x,0.25f),botRight.y).FlipY()),//Links
			SRSUtilities.RectAround(new Vector2(Mathf.Lerp(topLeft.x,botRight.x,0.75f),topLeft.y).FlipY(),botRight.FlipY())//Rechts
        };
    }
    ...
    public static void DrawBlackouts(bool adjusted = false)
    {
        foreach (Blackout b in blackouts)
        {
            ...
            {//Lasso blackouts have their own meshes on objects of which the 3D engine will take care.
                Rect r = adjusted ? new Rect(b.rect.x * Settings.ScreenW / Screen.width, b.rect.y * Settings.ScreenH / Screen.height, b.rect.width * Settings.ScreenW / Screen.width, b.rect.height * Settings.ScreenH / Screen.height) : b.rect;
                Graphics.DrawColoredTexture(r, b.elliptical ? Graphics.filledEllipse : Graphics.weiss1x1, b.farbe.WithAlpha(Blackout.selected > -1 && b == blackouts[Blackout.selected] ? selectedBlackoutColor.a : 1));
    ...
    public enum Mode { normal, save, load };
        ...
        switch (mode)
        {
            case Mode.save:
                GUI.Window(0, Menu.windowPosition, (id) =>
                {
                    ...
                    saveName = GUI.TextField(new Rect(8, Menu.saveWindowSize.y * 0.5f - 16, Menu.saveWindowSize.x - 16, 32), saveName);
                    ...
                    saveAsDefault = GUI.Toggle(new Rect(16, Menu.saveWindowSize.y * 0.5f + 32, 16, 16), saveAsDefault == (int) Settings.monitorMode,
                        "Set as Default " + (new string[] { "Gantry", "Wall" }[(int) Settings.monitorMode]) + " Configuration") ? (int) Settings.monitorMode : -1;
                    //						saveName=GUI.TextField(new Rect(16+8+8,Menu.saveWindowSize.y*0.5f+32,Menu.saveWindowSize.x-16,32),saveName);
                    if (GUI.Button(new Rect(8, Menu.saveWindowSize.y - 8 - 32, Menu.saveWindowSize.x * 0.5f - 16, 32), "Cancel"))
                    {
                        mode = Mode.normal;
                        Toolbar.clickedThisFrame = true;
                    }
                    if (GUI.Button(new Rect(Menu.saveWindowSize.x * 0.5f + 8, Menu.saveWindowSize.y - 8 - 32, Menu.saveWindowSize.x * 0.5f - 16, 32), "Save")/*&&!Menu.DraggingWindow*/)
                    {
                ...
                break;
            case Mode.load:
                if (fileToDelete != "") GUI.Window(0, Menu.windowPosition, (id) =>
                {
                    //string[] files=Directory.GetFiles(Settings.dataPath,"*.gantry");
                    GUI.Label(new Rect(0, 32, Menu.windowPosition.width - 8, Menu.windowPosition.height - 8 - 64 - 8), "Really delete the \"" + Path.GetFileNameWithoutExtension(fileToDelete) + "\" configuration?", gantrySkin.customStyles[2]);
                    if (GUI.Button(new Rect(8, Menu.saveWindowSize.y - 8 - 32, Menu.saveWindowSize.x * 0.5f - 16, 32), "No")) fileToDelete = "";
                    else if (GUI.Button(new Rect(Menu.saveWindowSize.x * 0.5f + 8, Menu.saveWindowSize.y - 8 - 32, Menu.saveWindowSize.x * 0.5f - 16, 32), "Yes"))
                    {
                        Debug.Log("Moving \"" + fileToDelete + "\" to \"" + backupDir + "/" + Path.GetFileName(fileToDelete) + "\".");
                        File.Move(fileToDelete, backupDir + "/" + Path.GetFileName(fileToDelete));
                        fileToDelete = "";
                    }
                }, "Load a Configuration");
                else GUI.Window(0, Menu.windowPosition, (id) =>
                {
                    string[] files = Directory.GetFiles(Settings.dataPath, "*.gantry");
                    Debug.Log(files.Length + " files.");
                    scrollPosition = GUI.BeginScrollView(new Rect(0, 32, Menu.windowPosition.width - 8, Menu.windowPosition.height - 8 - 64 - 8), scrollPosition, new Rect(0, 0, Menu.saveWindowSize.x - 32, files.Length * 40));
                    for (int i = 0; i < files.Length; i++)
                        ...
                        else if (GUI.Button(new Rect(8 + Menu.saveWindowSize.x - 64 - 8, i * 40, 32, 32), "X")) fileToDelete = files[i];
                    GUI.EndScrollView();
                    if (GUI.Button(new Rect(8, Menu.saveWindowSize.y - 8 - 32, Menu.saveWindowSize.x * 0.5f - 16, 32), "Cancel"))
                    {
                        mode = Mode.normal;
            ...
            case Mode.normal:
                ...
                for (int i = 0; i < mirror.Length; i++) if (mirror[i]) GUI.DrawTexture(new Rect((1 - i) * (Screen.width * (0.5f - 0.25f * (Projection.numScreens - 1) * Mathf.Sign(Camera.main.transform.position.x)) - 1), i * (Screen.height * 0.5f - 1), (1 - i) * 4 + i * Screen.width, i * 4 + (1 - i) * Screen.height), Graphics.weiss1x1);
                GUI.color = Color.white;
                if (toolBehaviour[(int) toolMode].Draw != null) toolBehaviour[(int) toolMode].Draw(SRSUtilities.adjustedFlipped);
    ...
    private static void SaveConfiguration(string fileName)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < verts.Length; i++) for (int j = 0; j < 3; j++) bw.Write((double) verts[i][j]);
            ...
            for (int i = 0; i < blackouts.Count; i++)
            {
                ...
                if (blackouts[i].lassoObject != null)
                {
                    ...
                    for (int v = 1; v < m.vertexCount; v++) for (int c = 0; c < 3; c++) bw.Write(m.vertices[v][c]);//We omit the first which is the center point which we can reconstruct from the remainder.
                }
                else bw.Write(0);
            }
    ...
    public static void LoadConfiguration(string fileName, int screen = -1)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < verts.Length; i++) for (int j = 0; j < 3; j++) verts[i][j] = (float) br.ReadDouble();
            ...
            for (int i = 0; i < numBlackouts; i++)
            {
                ...
                Rect r = new Rect((float) br.ReadDouble(), (float) br.ReadDouble(), (float) br.ReadDouble(), (float) br.ReadDouble());
                Blackout bo = new Blackout { elliptical = ellip, rect = r, farbe = c };
                int numLassoPoints = br.ReadInt32();
                if (numLassoPoints > 0)
                {
                    lassoPoints.Clear();
                    for (int l = 0; l < numLassoPoints; l++) lassoPoints.Add(new Vector3(br.ReadSingle(), br.ReadSingle(), br.ReadSingle()));
            ...
            Debug.Log("Read " + verts.Length + " vertices (" + columns + " columns) and " + numBlackouts + " blackouts (using " + blackouts.Count + ") from file " + fileName + ".");
        }
        catch (IOException e)
        {
    ...
    {
        Debug.Log("UnitTest: " + Mathf.InverseLerp(1, 2, 1.2f) + "," + Mathf.InverseLerp(1, 2, 0.8f) + "," + Mathf.InverseLerp(1, 2, 3));
        Debug.Log("UnitTest: " + Mathf.Lerp(0, 5, 0.2f) + "," + Mathf.Lerp(0, 5, -0.2f) + ",MyLerp: " + SRSUtilities.Lerp(0, 5, -0.2f));
    }
}
